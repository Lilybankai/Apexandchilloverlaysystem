  ...
  const STEPS = [
    ...
    },
    {
      ...
      points: [
        ...
        'Switch the map to Inputs and the line is coloured by the pedals instead — red braking, green on the throttle — with a bar across each line where that lap first braked for the corner. Two bars a few metres apart is who brakes later; the readout and the strip under the map put a number on it.',
        'Your line is the wider of the two and the other driver’s is finer, each with a thread of its own colour down the middle, so you can still tell them apart where they run on top of each other. The tick boxes beside Pace | Inputs take either line off the map altogether; nothing is unloaded, and the charts, the delta and the braking strip still have both laps. D | E | F beside them tries three ways of weighting the two lines — say which reads best and the other two go.',
