/**
 * data/racedata.js — Le Mans Ultimate track/car reference data + the pure
 * pre-race fuel-strategy engine, for the Fuel Planner widget (widgets/fuelplan.js).
 * -----------------------------------------------------------------------------
 * Ported verbatim from the standalone Fuel Calculator app
 * (src/lib/raceData.js) and stripped of its ES-module / optional-chaining
 * syntax so it runs as a plain <script> in OBS's CEF alongside the other
 * overlay modules. Load this BEFORE widgets/fuelplan.js (defer preserves order).
 *
 * Exposes `window.ApexRaceData`:
 *   CIRCUITS, CAR_CLASSES, CARS, DEFAULT_LAP_TIMES  — the data tables
 *   getCircuit, getLayout, getDefaultLapTime, getCarData, getDefaultFuelPerLap,
 *   calculateFuel, formatLapTime                    — the engine + helpers
 *
 * `calculateFuel` is a pure timed-race model: race minutes -> laps (rounded up)
 * -> base fuel + safety-lap margin -> total, pit stops and laps-per-stint. It is
 * the OFFLINE planner; live consumption/VE come from telemetry in the widget.
 */
(function () {
  "use strict";
  ...
  var CIRCUITS = [
    {
      ...
      layouts: [{ id: "portimao_gp", name: "Grand Prix", length: 4.653 }],
    },
    {
      id: "imola",
      ...
      dlc: "2024 Pack 1",
      layouts: [{ id: "imola_gp", name: "Grand Prix", length: 4.909 }],
    },
    {
      id: "monza",
    ...
    },
    {
      ...
      dlc: "2024 Pack 3",
      layouts: [{ id: "interlagos_gp", name: "Grand Prix", length: 4.309 }],
    },
    {
      id: "bahrain",
    ...
    },
    {
      ...
      dlc: "ELMS Pack 3",
      layouts: [{ id: "barcelona_gp", name: "Grand Prix", length: 4.657 }],
    },
    {
      id: "lemans",
    ...
    },
    {
      ...
      dlc: "2024 Pack 5",
      layouts: [
        { id: "qatar_gp", name: "Grand Prix", length: 5.4 },
    ...
    },
    {
      ...
      dlc: "ELMS Pack 1",
      layouts: [
        { id: "silverstone_gp_wec", name: "Grand Prix Circuit (WEC)", length: 5.89 },
  ...
  var CAR_CLASSES = [
    { id: "hypercar", name: "Hypercar", color: "#FF3333" },
    { id: "lmp2", name: "LMP2", color: "#0088FF" },
    { id: "lmp3", name: "LMP3", color: "#00CCAA" },
    { id: "gte", name: "GTE", color: "#FFB800" },
    { id: "lmgt3", name: "LMGT3", color: "#BB44FF" },
  ];
  ...
  var CARS = {
    ...
    lmp2: [
      { id: "oreca_07", name: "Oreca 07 Gibson", fuelCapacity: 63, avgConsumption: 3.4, consumptionByTrack: {
      ...
      { id: "oreca_07_elms", name: "Oreca 07 Gibson ELMS", fuelCapacity: 63, avgConsumption: 3.4 },
    ],
    lmp3: [
  ...
  function getCircuit(circuitId) {
    for (var i = 0; i < CIRCUITS.length; i++) {
      if (CIRCUITS[i].id === circuitId) return CIRCUITS[i];
    }
    return null;
  }
  ...
  function getLayout(circuitId, layoutId) {
    ...
    for (var i = 0; i < circuit.layouts.length; i++) {
      if (circuit.layouts[i].id === layoutId) return circuit.layouts[i];
    }
    return null;
  }

  function getDefaultLapTime(layoutId, classId) {
    var row = DEFAULT_LAP_TIMES[layoutId];
    return (row && row[classId]) || null;
  }

  function getCarData(classId, carId) {
    var list = CARS[classId];
    if (!list) return null;
    for (var i = 0; i < list.length; i++) {
      if (list[i].id === carId) return list[i];
    }
    return null;
  }

  function getDefaultFuelPerLap(classId, carId, layoutId) {
    var car = getCarData(classId, carId);
    if (!car) return null;
    var byTrack = car.consumptionByTrack ? car.consumptionByTrack[layoutId] : undefined;
    return byTrack != null ? byTrack : car.avgConsumption;
  }

  /**
   * Pure timed-race fuel plan. Given track/car identity plus race minutes (and
   * optional overrides), returns total fuel, lap count, pit stops and
   * laps-per-stint. Returns `null` when the track/car/lap-time can't be resolved.
   * Mirrors the standalone calculator exactly (rounding conventions included).
   */
  function calculateFuel(opts) {
    var circuitId = opts.circuitId;
    var layoutId = opts.layoutId;
    var classId = opts.classId;
    var carId = opts.carId;
    var raceMinutes = opts.raceMinutes;
    var customFuelPerLap = opts.customFuelPerLap;
    var customLapTime = opts.customLapTime;
    var safetyLaps = opts.safetyLaps;
    var customTankCapacity = opts.customTankCapacity;

    var circuit = getCircuit(circuitId);
    var layout = getLayout(circuitId, layoutId);
    var car = getCarData(classId, carId);
    var defaultLapTime = getDefaultLapTime(layoutId, classId);

    if (!circuit || !layout || !car || !defaultLapTime) return null;

    var lapTimeSec = customLapTime || defaultLapTime;
    var byTrack = car.consumptionByTrack ? car.consumptionByTrack[layoutId] : undefined;
    var fuelPerLap = customFuelPerLap || byTrack || car.avgConsumption;
    // LMP2 at Le Mans has a 75L tank cap; otherwise use car default.
    var isLemans = layoutId === "lemans_full" || layoutId === "lemans_mulsanne";
    var defaultTankCap = classId === "lmp2" && isLemans ? 75 : car.fuelCapacity;
    var tankCap = customTankCapacity || defaultTankCap;
    var raceSeconds = raceMinutes * 60;
    var totalLapsExact = Math.round((raceSeconds / lapTimeSec) * 10) / 10;
    var totalLaps = Math.ceil(raceSeconds / lapTimeSec);
    var baseFuel = totalLaps * fuelPerLap;
    var safetyFuel = (safetyLaps || 0) * fuelPerLap;
    var totalFuel = baseFuel + safetyFuel;
    var fuelStops = Math.max(0, Math.ceil(totalFuel / tankCap) - 1);
    var fuelPerStint = tankCap;
    var lapsPerStint = fuelPerLap > 0 ? Math.floor(fuelPerStint / fuelPerLap) : 0;

    return {
      trackName: circuit.name + " — " + layout.name,
      totalLapsExact: totalLapsExact,
      carName: car.name,
      fuelCapacity: tankCap,
      lapTimeSec: lapTimeSec,
      fuelPerLap: fuelPerLap,
      totalLaps: totalLaps,
      baseFuel: Math.ceil(baseFuel * 10) / 10,
      safetyFuel: Math.ceil(safetyFuel * 10) / 10,
      totalFuel: Math.ceil(totalFuel * 10) / 10,
      fuelStops: fuelStops,
  ...
  function formatLapTime(seconds) {
    ...
    // Pad the seconds to two integer digits (e.g. "1:04.3") so times align.
    if (String(secs).indexOf(".") === 1) secs = "0" + secs;
    return mins + ":" + secs;
  }
  ...
  window.ApexRaceData = {
    ...
    CARS: CARS,
    DEFAULT_LAP_TIMES: DEFAULT_LAP_TIMES,
    getCircuit: getCircuit,
    getLayout: getLayout,
    getDefaultLapTime: getDefaultLapTime,
    getCarData: getCarData,
    getDefaultFuelPerLap: getDefaultFuelPerLap,
